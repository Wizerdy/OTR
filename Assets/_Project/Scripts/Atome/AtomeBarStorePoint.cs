...
public class AtomeBarStorePoint : MonoBehaviour {
    ...
    void Start() {
        _entityStorePoint.CurrentValueChanged += (float value) => { if (value < _atomeBar.Value) { _atomeBar.Remove(Mathf.Abs(value - _atomeBar.Value)); } else { _atomeBar.Add(Mathf.Abs(value - _atomeBar.Value)); } };
    ...
    private void OnDestroy() {
        _entityStorePoint.CurrentValueChanged -= (float value) => { if (value < _entityStorePoint.CurrentValue) { _atomeBar.Remove(Mathf.Abs(value - _entityStorePoint.CurrentValue)); } else { _atomeBar.Add(Mathf.Abs(value - _entityStorePoint.CurrentValue)); } };
        _entityStorePoint.MaxValueChanged -= (float value) => _atomeBar.ChangeMaxValue(value);
        _entityStorePoint.MinValueChanged -= (float value) => _atomeBar.ChangeMinValue(value);
